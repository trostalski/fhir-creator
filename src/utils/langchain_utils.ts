...
export const promptList = {
  ...
  correctValidationError: ChatPromptTemplate.fromPromptMessages([
    HumanMessagePromptTemplate.fromTemplate(
      templateList.correctValidationError
    ),
  ]),
};
...
const checkFailCap = (
  ...
  if (failCount >= failCap) {
    toastError(
      "Exceeded the maximum number of tries while creating a resource"
    );
...
const createBuildCall = async (
  ...
  ({ jsonResult, failCount } = await jsonTransformation(
    response,
    inputDict,
    failCount,
    failCap,
  ...
  ({ jsonResult, failCount } = await fhirValidation(
    jsonResult,
    inputDict,
    failCount,
    failCap,
...
const getErrorLocation = (errorMessages: ValidatorMessage[]) => {
  const arrayErrorIndex = errorMessages.findIndex(
    (message) => message.message === "Property is not an array"
  );
...
const fhirValidation = async (
  ...
  if (valResult.valid) {
    ...
    const errorMessages = valResult.messages.filter(
      (message) => message.severity === "error"
    );
    if (checkArrayErrorSolvable(errorMessages)) {
      // handle array error
      const errorLocation = getErrorLocation(errorMessages);
      ...
      failCount += 1;
      const retryInputDict = createValidationFailInputDict(
        inputDict,
        jsonResult,
        errorMessages
      );
...
const createValidationFailInputDict = (
  inputDict: InputDict,
  jsonResult: any,
...
const injectRefs = (jsonResult: any) => {
  if (
    ...
    (jsonResult["resourceType"] === "Condition" ||
      jsonResult["resourceType"] === "Observation")
  ) {
    jsonResult["subject"] = { reference: "Patient/example" };
  }
};

const jsonTransformation = async (
